...
#not
execute if score dummy Inqueue matches 4.. run scoreboard players add dummy Getqueue 1
#can 
execute unless score dummy Inqueue matches 4.. run scoreboard players add dummy Getqueue 0
execute if score dummy Inqueue matches ..3 if score dummy Getqueue matches 0 run tag @e[family=customer1_waiting,tag=!queue,c=1,x=13,y=-24,z=134,dx=100,dy=24,dz=60] add queue
